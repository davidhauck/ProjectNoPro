...
function beginGoogleAuth() {
  $.ajax({
    url: localStorage.host.concat('api/Account/ExternalLogins?returnUrl=/&generateState=true'),
    type: "GET",
...
function onOauthUrlsFound(res) {
  ...
  var authWindow = gui.Window.open(localStorage.host.concat(googleUrl.Url), {}, function (new_win) {
...
function accessTokenObtained() {
  $.ajax({
    url: localStorage.host.concat('api/Account/UserInfo'),
    type: "GET",
...
function registerUser(account) {
  ...
  $.ajax({
    url: localStorage.host.concat('api/Account/RegisterExternal'),
    data: theData,
